...
def get_head(cfg):
    ...
    x = []
    in_size = cfg['REPR_SIZE']
    for _ in range(cfg['HEAD']['LAYERS'] - 1):
        x.append(nn.Linear(in_size, cfg['HEAD']['SIZE']))
        if cfg.add_bn:
            x.append(nn.BatchNorm1d(cfg['HEAD']['SIZE']))
        x.append(nn.ReLU())
        in_size = cfg['HEAD']['SIZE']
    x.append(nn.Linear(in_size, cfg['EMB_SIZE']))
